...
[English](SNAPSHOTS_AND_SCANNING.md) | 日本語
...
コマンドラインで指定しなかった項目には、プロファイル設定または既定値が使われます。全オプションと既定値は、後半の[`qdu snapshot`オプション一覧](COMMAND_REFERENCE_ja.md#qdu-snapshot)で確認できます。
